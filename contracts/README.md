...
| `factory-report.v1` | 입력 — **공장 MES 일 1회 집계**(T1+T2). `is_missing` = API 끊김 | 공장 MES/FactoryOS API · **유형 a 만** | `factory_reports.jsonl` |
| `fai-report.v1` | 입력 — 초도품 검사(AS9102 Form 3 요약). 유형 무관 | 공장 / 우리 대행 / 제3자 | `fai_reports.jsonl` |
| `cell-daily.v1` | 입력 — 설비 텔레메트리 일 집계. **교차검증용** | CellOS · **유형 a·b** | `cell_daily.jsonl` |
...
- **결측은 값이 아니라 상태다.** `factory-report.v1` 의 `is_missing=true` 행은 수량 필드를 가질 수 없다(`if/then`). MES 가 없는 공장은 행 자체가 없다.
