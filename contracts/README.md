# 계약 스키마 — 입력 6종 · 출력 1종 (초안, 2026-09-08)
...
| `factory-report.v1` | 입력 — **공장 MES 집계**(주기 공장별: 일/시프트/주). `is_missing` = API 끊김. 자재·잔업은 ERP 로 | MES → FactoryOS · **12곳 전부** | `factory_reports.jsonl` |
| `erp-daily.v1` | 입력 — **공장 ERP 일 스냅샷**: 자재 입출고·재고 · 재공/공정별 재고 · 인원·잔업 · 외주. MES 의 대조군 | ERP → FactoryOS · 12곳 전부 (09-08 저녁) | `erp_daily.jsonl` |
| `fai-report.v1` | 입력 — 초도품 검사(AS9102 Form 3 요약). 유형 무관 | 공장 / 우리 대행 / 제3자 | `fai_reports.jsonl` |
| `cell-daily.v1` | 입력 — 설비 텔레메트리 일 집계. **카운터 = 실제 생산** | CellOS · 12곳 전부 | `cell_daily.jsonl` |
| `order-quality-outcome.v1` | **라벨** — 입고검사(표본 n 중 불합격 k) + 클레임. `label_available_at` 이 핵심 | 우리 · 고객 검사 | `quality_outcomes.jsonl` |
| `supplier-risk-score.v1` | **출력** — 제조 품질(내부 불량률) · 검수 품질(검출률) · 보고 정직도 · 예상 유출 · 불일치 · 검토 필요 | 모델 | `scores.jsonl` |
...
- **결측은 값이 아니라 상태다.** `factory-report.v1` · `erp-daily.v1` 의 `is_missing=true` 행은 수량 필드를 가질 수 없다(`if/then`). 공장이 채우지 않는 선택 필드는 키 자체가 없다.
- **`label_available_at`** — 결과를 우리가 알게 된 시각. 모델은 이 시각 이전에 그 행을 볼 수 없다. `market` 이 이 지연을 정한다(18/30/60일).
- 파생값은 계약에 넣지 않는다 — `lead_slack` `price_zscore` 는 원값에서 피처 빌더가 만든다.
...
```
pytest tests/unit/test_contracts.py     # 생성 레코드 전부 × 계약 7종
```
