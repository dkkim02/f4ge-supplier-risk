...
2026-09-10 부터 생성기는 주관적 공장 차이(보고 편향·노이즈·성실도)를 **통제**한다.
그 통제 자체가 가정이므로, 편향 관련 축은 **통제값에서 벗어나는 방향으로 되돌려 보는 강건성 검사**가 된다.
각 축의 값 목록에 통제값을 넣어 두었으니 「←현재」가 붙는 행이 지금 세계다.

생성기 파라미터는 실측 근거가 없다(docs/생성기_캘리브레이션.md §2.3~2.4).
...
SWEEPS: dict[str, tuple[str, tuple]] = {
    ...
    "report_bias_mean": ("보고 편향 크기", (0.3, 0.5, 0.7, 0.9, 1.0)),  # 1.0 = 09-10 통제값
    ...
    "mes_input_floor": ("MES 입력 정직도 하한", (0.02, 0.3, 0.55, 0.8, 1.0)),  # 1.0 = 09-10 통제값
}
# 09-10 오후에 뺀 축 — `bias_capability_corr`. 편향 통제로 report_bias_sd 가 0 이 되면서
# 상관이 걸릴 분산 자체가 사라졌다(4개 값 전부 rb 1.00/sd 0.00, 완전 무작용).
# 판정은 이미 끝났다(커밋 0732bfd): 0 → -0.85 에서 예측·불일치 어느 쪽도 안 흔들렸다.
